...
function onClickSendPublicMessage(e) {
  ...
  // Send the message
  io.socket.post('/chatm/public', {room: roomId, msg: message});
...
function joinRoom() {
  ...
  // Join the room
  io.socket.post('/roomm/'+roomId+'/users', {id: window.me.id});
...
function onClickLeaveRoom(e) {
  ...
  io.socket.delete('/roomm/'+roomId+'/users', {id: window.me.id});
